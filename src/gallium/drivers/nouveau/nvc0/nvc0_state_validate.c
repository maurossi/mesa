...
nvc0_validate_zcull(struct nvc0_context *nvc0)
{
    ...
    BEGIN_NVC0(&screen->base, push, NVC0_3D(ZCULL_REGION), 1);
    PUSH_DATA (&screen->base, push, 0);
    BEGIN_NVC0(&screen->base, push, NVC0_3D(ZCULL_ADDRESS_HIGH), 2);
    PUSH_DATAh(&screen->base, push, bo->offset + offset);
    PUSH_DATA (&screen->base, push, bo->offset + offset);
    offset += 1 << 17;
    BEGIN_NVC0(&screen->base, push, NVC0_3D(ZCULL_LIMIT_HIGH), 2);
    PUSH_DATAh(&screen->base, push, bo->offset + offset);
    PUSH_DATA (&screen->base, push, bo->offset + offset);
    BEGIN_NVC0(&screen->base, push, SUBC_3D(0x07e0), 2);
    PUSH_DATA (&screen->base, push, size);
    PUSH_DATA (&screen->base, push, size >> 16);
    BEGIN_NVC0(&screen->base, push, SUBC_3D(0x15c8), 1); /* bits 0x3 */
    PUSH_DATA (&screen->base, push, 2);
    BEGIN_NVC0(&screen->base, push, NVC0_3D(ZCULL_WIDTH), 4);
    PUSH_DATA (&screen->base, push, width);
    PUSH_DATA (&screen->base, push, height);
    PUSH_DATA (&screen->base, push, 1);
    PUSH_DATA (&screen->base, push, 0);
    BEGIN_NVC0(&screen->base, push, NVC0_3D(ZCULL_WINDOW_OFFSET_X), 2);
    PUSH_DATA (&screen->base, push, 0);
    PUSH_DATA (&screen->base, push, 0);
    BEGIN_NVC0(&screen->base, push, NVC0_3D(ZCULL_INVALIDATE), 1);
    PUSH_DATA (&screen->base, push, 0);
}
#endif

static inline void
nvc0_fb_set_null_rt(struct nvc0_screen *screen, struct nouveau_pushbuf *push, unsigned i, unsigned layers)
{
   BEGIN_NVC0(&screen->base, push, NVC0_3D(RT_ADDRESS_HIGH(i)), 9);
   PUSH_DATA (&screen->base, push, 0);
   PUSH_DATA (&screen->base, push, 0);
   PUSH_DATA (&screen->base, push, 64);     // width
   PUSH_DATA (&screen->base, push, 0);      // height
   PUSH_DATA (&screen->base, push, 0);      // format
   PUSH_DATA (&screen->base, push, 0);      // tile mode
   PUSH_DATA (&screen->base, push, layers); // layers
   PUSH_DATA (&screen->base, push, 0);      // layer stride
   PUSH_DATA (&screen->base, push, 0);      // base layer
}
...
gm200_validate_sample_locations(struct nvc0_context *nvc0, unsigned ms)
{
   ...
   PUSH_DATAh(&screen->base, push, screen->uniform_bo->offset + NVC0_CB_AUX_INFO(4));
   PUSH_DATA (&screen->base, push, screen->uniform_bo->offset + NVC0_CB_AUX_INFO(4));
   BEGIN_1IC0(&screen->base, push, NVC0_3D(CB_POS), 1 + 64);
   PUSH_DATA (&screen->base, push, NVC0_CB_AUX_SAMPLE_INFO);
   ...
   PUSH_DATAp(&screen->base, push, cb, 64);
   ...
   BEGIN_NVC0(&screen->base, push, SUBC_3D(0x11e0), 4);
   PUSH_DATAp(&screen->base, push, packed_locations, 4);
}
...
nvc0_validate_sample_locations(struct nvc0_context *nvc0, unsigned ms)
{
   ...
   PUSH_DATAh(&screen->base, push, screen->uniform_bo->offset + NVC0_CB_AUX_INFO(4));
   PUSH_DATA (&screen->base, push, screen->uniform_bo->offset + NVC0_CB_AUX_INFO(4));
   BEGIN_1IC0(&screen->base, push, NVC0_3D(CB_POS), 1 + 2 * ms);
   PUSH_DATA (&screen->base, push, NVC0_CB_AUX_SAMPLE_INFO);
   for (i = 0; i < ms; i++) {
      float xy[2];
      nvc0->base.pipe.get_sample_position(&nvc0->base.pipe, ms, i, xy);
      PUSH_DATAf(&screen->base, push, xy[0]);
      PUSH_DATAf(&screen->base, push, xy[1]);
   }
}
...
nvc0_validate_fb(struct nvc0_context *nvc0)
{
   ...
   PUSH_DATA (&screen->base, push, fb->width << 16);
   PUSH_DATA (&screen->base, push, fb->height << 16);
   ...
   for (i = 0; i < fb->nr_cbufs; ++i) {
      ...
      if (!fb->cbufs[i]) {
         nvc0_fb_set_null_rt(nvc0->screen, push, i, 0);
         continue;
      }
      ...
      if (likely(nouveau_bo_memtype(bo))) {
         ...
         PUSH_DATA(&screen->base, push, sf->base.u.tex.first_layer + sf->depth);
         PUSH_DATA(&screen->base, push, mt->layer_stride >> 2);
         PUSH_DATA(&screen->base, push, sf->base.u.tex.first_layer);
         ...
         if (res->base.target == PIPE_BUFFER) {
            ...
            PUSH_DATA(&screen->base, push, nv50_miptree(sf->base.texture)->level[0].pitch);
            PUSH_DATA(&screen->base, push, sf->height);
         }
         PUSH_DATA(&screen->base, push, nvc0_format_table[sf->base.format].rt);
   ...
   if (fb->zsbuf) {
      ...
      PUSH_DATA (&screen->base, push, nvc0_format_table[fb->zsbuf->format].rt);
      ...
      PUSH_DATA (&screen->base, push, sf->width);
      PUSH_DATA (&screen->base, push, sf->height);
      PUSH_DATA (&screen->base, push, (unk << 16) |
   ...
   if (nr_cbufs == 0 && !fb->zsbuf) {
      ...
      nvc0_fb_set_null_rt(nvc0->screen, push, 0, fb->layers);
   ...
   PUSH_DATA (&screen->base, push, (076543210 << 4) | nr_cbufs);
   IMMED_NVC0(&screen->base, push, NVC0_3D(MULTISAMPLE_MODE), ms_mode);
...
nvc0_validate_blend_colour(struct nvc0_context *nvc0)
{
   ...
   BEGIN_NVC0(&screen->base, push, NVC0_3D(BLEND_COLOR(0)), 4);
   PUSH_DATAf(&screen->base, push, nvc0->blend_colour.color[0]);
   PUSH_DATAf(&screen->base, push, nvc0->blend_colour.color[1]);
   PUSH_DATAf(&screen->base, push, nvc0->blend_colour.color[2]);
   PUSH_DATAf(&screen->base, push, nvc0->blend_colour.color[3]);
}
...
nvc0_validate_stencil_ref(struct nvc0_context *nvc0)
{
   ...
   struct nvc0_screen *screen = nvc0->screen;
    ...
    IMMED_NVC0(&screen->base, push, NVC0_3D(STENCIL_FRONT_FUNC_REF), ref[0]);
    IMMED_NVC0(&screen->base, push, NVC0_3D(STENCIL_BACK_FUNC_REF), ref[1]);
}
...
nvc0_validate_stipple(struct nvc0_context *nvc0)
{
   ...
   struct nvc0_screen *screen = nvc0->screen;
    ...
    BEGIN_NVC0(&screen->base, push, NVC0_3D(POLYGON_STIPPLE_PATTERN(0)), 32);
    for (i = 0; i < 32; ++i)
        PUSH_DATA(&screen->base, push, util_bswap32(nvc0->stipple.stipple[i]));
}
...
nvc0_validate_scissor(struct nvc0_context *nvc0)
{
   ...
   for (i = 0; i < NVC0_MAX_VIEWPORTS; i++) {
      ...
      BEGIN_NVC0(&screen->base, push, NVC0_3D(SCISSOR_HORIZ(i)), 2);
      if (nvc0->rast->pipe.scissor) {
         PUSH_DATA(&screen->base, push, (s->maxx << 16) | s->minx);
         PUSH_DATA(&screen->base, push, (s->maxy << 16) | s->miny);
      } else {
         PUSH_DATA(&screen->base, push, (0xffff << 16) | 0);
         PUSH_DATA(&screen->base, push, (0xffff << 16) | 0);
      }
   }
...
nvc0_validate_viewport(struct nvc0_context *nvc0)
{
   ...
   for (i = 0; i < NVC0_MAX_VIEWPORTS; i++) {
      ...
      BEGIN_NVC0(&screen->base, push, NVC0_3D(VIEWPORT_TRANSLATE_X(i)), 3);
      PUSH_DATAf(&screen->base, push, vp->translate[0]);
      PUSH_DATAf(&screen->base, push, vp->translate[1]);
      PUSH_DATAf(&screen->base, push, vp->translate[2]);

      BEGIN_NVC0(&screen->base, push, NVC0_3D(VIEWPORT_SCALE_X(i)), 3);
      PUSH_DATAf(&screen->base, push, vp->scale[0]);
      PUSH_DATAf(&screen->base, push, vp->scale[1]);
      PUSH_DATAf(&screen->base, push, vp->scale[2]);
      ...
      BEGIN_NVC0(&screen->base, push, NVC0_3D(VIEWPORT_HORIZ(i)), 2);
      PUSH_DATA (&screen->base, push, (w << 16) | x);
      PUSH_DATA (&screen->base, push, (h << 16) | y);
      ...
      PUSH_DATAf(&screen->base, push, zmin);
      PUSH_DATAf(&screen->base, push, zmax);

      if (class_3d >= GM200_3D_CLASS) {
         BEGIN_NVC0(&screen->base, push, NVC0_3D(VIEWPORT_SWIZZLE(i)), 1);
         PUSH_DATA (&screen->base, push, vp->swizzle_x << 0 |
                          vp->swizzle_y << 4 |
                          vp->swizzle_z << 8 |
                          vp->swizzle_w << 12);
      }
...
nvc0_validate_window_rects(struct nvc0_context *nvc0)
{
   ...
   IMMED_NVC0(&screen->base, push, NVC0_3D(CLIP_RECTS_EN), enable);
   if (!enable)
      return;

   IMMED_NVC0(&screen->base, push, NVC0_3D(CLIP_RECTS_MODE), !nvc0->window_rect.inclusive);
   BEGIN_NVC0(&screen->base, push, NVC0_3D(CLIP_RECT_HORIZ(0)), NVC0_MAX_WINDOW_RECTANGLES * 2);
...
nvc0_upload_uclip_planes(struct nvc0_context *nvc0, unsigned s)
{
   ...
   BEGIN_1IC0(&screen->base, push, NVC0_3D(CB_POS), PIPE_MAX_CLIP_PLANES * 4 + 1);
   PUSH_DATA (&screen->base, push, NVC0_CB_AUX_UCP_INFO);
   PUSH_DATAp(&screen->base, push, &nvc0->clip.ucp[0][0], PIPE_MAX_CLIP_PLANES * 4);
}
...
nvc0_validate_clip(struct nvc0_context *nvc0)
{
   ...
   if (nvc0->state.clip_enable != clip_enable) {
      ...
      IMMED_NVC0(&screen->base, push, NVC0_3D(CLIP_DISTANCE_ENABLE), clip_enable);
   }
   if (nvc0->state.clip_mode != vp->vp.clip_mode) {
      nvc0->state.clip_mode = vp->vp.clip_mode;
      BEGIN_NVC0(&screen->base, push, NVC0_3D(CLIP_DISTANCE_MODE), 1);
      PUSH_DATA (&screen->base, push, vp->vp.clip_mode);
   }
}
...
nvc0_validate_blend(struct nvc0_context *nvc0)
{
   ...
   PUSH_SPACE(&screen->base, push, nvc0->blend->size);
   PUSH_DATAp(&screen->base, push, nvc0->blend->state, nvc0->blend->size);
}
...
nvc0_validate_zsa(struct nvc0_context *nvc0)
{
   ...
   PUSH_SPACE(&screen->base, push, nvc0->zsa->size);
   PUSH_DATAp(&screen->base, push, nvc0->zsa->state, nvc0->zsa->size);
}
...
nvc0_validate_rasterizer(struct nvc0_context *nvc0)
{
   ...
   PUSH_SPACE(&screen->base, push, nvc0->rast->size);
   PUSH_DATAp(&screen->base, push, nvc0->rast->state, nvc0->rast->size);
}
...
nvc0_validate_buffers(struct nvc0_context *nvc0)
{
   ...
   for (s = 0; s < 5; s++) {
      ...
      PUSH_DATAh(&screen->base, push, screen->uniform_bo->offset + NVC0_CB_AUX_INFO(s));
      PUSH_DATA (&screen->base, push, screen->uniform_bo->offset + NVC0_CB_AUX_INFO(s));
      BEGIN_1IC0(&screen->base, push, NVC0_3D(CB_POS), 1 + 4 * NVC0_MAX_BUFFERS);
      PUSH_DATA (&screen->base, push, NVC0_CB_AUX_BUF_INFO(0));
...
nvc0_validate_sample_mask(struct nvc0_context *nvc0)
{
   ...
   PUSH_DATA (&screen->base, push, mask[0]);
   PUSH_DATA (&screen->base, push, mask[1]);
   PUSH_DATA (&screen->base, push, mask[2]);
   PUSH_DATA (&screen->base, push, mask[3]);
}
...
nvc0_validate_min_samples(struct nvc0_context *nvc0)
{
   ...
   IMMED_NVC0(&screen->base, push, NVC0_3D(SAMPLE_SHADING), samples);
}
...
nvc0_validate_fp_zsa_rast(struct nvc0_context *nvc0)
{
   ...
   if (rasterizer_discard != nvc0->state.rasterizer_discard) {
      ...
      IMMED_NVC0(&screen->base, push, NVC0_3D(RASTERIZE_ENABLE), !rasterizer_discard);
   }
}
...
nvc0_validate_zsa_fb(struct nvc0_context *nvc0)
{
   ...
   if (nvc0->zsa && nvc0->zsa->pipe.alpha.enabled &&
      ...
      nvc0_fb_set_null_rt(nvc0->screen, push, 0, 0);
      BEGIN_NVC0(&screen->base, push, NVC0_3D(RT_CONTROL), 1);
      PUSH_DATA (&screen->base, push, (076543210 << 4) | 1);
   }
}
...
nvc0_validate_rast_fb(struct nvc0_context *nvc0)
{
   ...
   if (rast->offset_units_unscaled) {
      BEGIN_NVC0(&screen->base, push, NVC0_3D(POLYGON_OFFSET_UNITS), 1);
      if (fb->zsbuf && fb->zsbuf->format == PIPE_FORMAT_Z16_UNORM)
         PUSH_DATAf(&screen->base, push, rast->offset_units * (1 << 16));
      else
         PUSH_DATAf(&screen->base, push, rast->offset_units * (1 << 24));
   }
}
...
nvc0_validate_tess_state(struct nvc0_context *nvc0)
{
   ...
   BEGIN_NVC0(&screen->base, push, NVC0_3D(TESS_LEVEL_OUTER(0)), 6);
   PUSH_DATAp(&screen->base, push, nvc0->default_tess_outer, 4);
   PUSH_DATAp(&screen->base, push, nvc0->default_tess_inner, 2);
}
...
nvc0_validate_fbread(struct nvc0_context *nvc0)
{
   ...
   if (new_view) {
      ...
      if (screen->base.class_3d >= NVE4_3D_CLASS) {
         ...
         PUSH_DATAh(&screen->base, push, screen->uniform_bo->offset + NVC0_CB_AUX_INFO(4));
         PUSH_DATA (&screen->base, push, screen->uniform_bo->offset + NVC0_CB_AUX_INFO(4));
         BEGIN_1IC0(&screen->base, push, NVC0_3D(CB_POS), 1 + 1);
         PUSH_DATA (&screen->base, push, NVC0_CB_AUX_FB_TEX_INFO);
         PUSH_DATA (&screen->base, push, (0 << 20) | tic->id);
      } else {
         BEGIN_NVC0(&screen->base, push, NVC0_3D(BIND_TIC2(0)), 1);
         PUSH_DATA (&screen->base, push, (tic->id << 9) | 1);
      }
                    ...
                    uint32_t *dirty, struct nouveau_bufctx *bufctx)
{
   ...
   return !PUSH_KICK(&nvc0->screen->base, nvc0->base.pushbuf);
}

...
static uint32_t
pipe_to_nv_texture_type(enum pipe_texture_target target)
{
#define CASE(PIPE, NV) \
   case PIPE_TEXTURE_##PIPE: return NVB097_TEXHEAD_BL_TEXTURE_TYPE_##NV; \
...
{
   if (view->target != PIPE_TEXTURE_3D && view->array_len == 1 &&
                          ...
                          void *desc_out)
{
   ...
   TH_NV9097_SET_U(th, 2, TEXTURE_TYPE,
                   pipe_to_nv_texture_type(view->target));
                          ...
                          void *desc_out)
{
   ...
   TH_NVB097_SET_U(th, BL, TEXTURE_TYPE,
                   pipe_to_nv_texture_type(view->target));
